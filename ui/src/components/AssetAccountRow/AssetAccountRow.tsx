...
export const AssetAccountRow: React.FC<AssetAccountRowProps> = ({
  reference,
  contractId,
  isFungible,
  issuer,
      ...
      <Card sx={{ marginBottom: 1 }}>
        <CardActionArea component={Link} to={assetPath}>
          <CardContent className={classes.root}>
            <Avatar sx={{ marginRight: 1 }}>{ticker[0]}</Avatar>
            <div>
              <Typography
                sx={{ fontSize: 14, marginRight: 1, fontWeight: "500" }}
                color="text.primary"
              >
                {ticker}
              </Typography>
            ...
            <Box marginLeft={"auto"} display="flex" alignItems="center">
              ...
              {!isMobile() && issuer === admin && ticker === "ET" && (
                <Button
                  sx={{ marginRight: 1 }}
                  variant="outlined"
                  size="small"
